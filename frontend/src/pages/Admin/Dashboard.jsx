...
const Dashboard = () => {
  ...
  const identifyLowStockProducts = (products) => {
    ...
    const productsWithLowStock = products
      .filter(product => product.totalQuantity > 0 && product.totalQuantity < lowStockThreshold)
  ...
  const statCards = [
    ...
    { title: 'Total Revenue', value: `Rs.${stats.revenue.toLocaleString('en-US', { minimumFractionDigits: 2 })}`, color: 'bg-yellow-500' },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <tbody className="divide-y divide-gray-200">
                  {lowStockProducts.map((product) => (
                    <tr key={product.id}>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          ...
                          <div className="truncate max-w-xs">
                            ...
                            <div className="text-xs text-gray-500">ID: #{product.id.substring(product.id.length - 6)}</div>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`px-2 py-1 rounded-full text-xs ${getStockBadgeClass(product.quantity)}`}>
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          ...
          {newProducts.length > 0 ? (
                ...
                <div key={product.id} className="flex items-center">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center text-gray-500 text-sm mt-1">
                      <span className={`px-2 py-1 rounded-full text-xs mr-2 ${getCategoryBadgeClass(product.category)}`}>
                        {product.category}
                      </span>
            ...
            <p className="text-gray-500 italic">No new products added recently</p>
          )}
        </div>
      </div>
